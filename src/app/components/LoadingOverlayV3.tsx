...
export default function LoadingOverlayV3({ onCancel }: LoadingOverlayProps) {
  ...
  return (
    <div className="fixed inset-0 flex flex-col items-center justify-center bg-black/90 backdrop-blur-sm z-[9999]">
      {/* CANCEL BUTTON */}
      {onCancel && (
        <button
          onClick={onCancel}
      ...
      {/* FLOATING JAR */}
      <div className="relative mb-10 animate-[float_6s_ease-in-out_infinite]">
        {/* Glow Effect */}
        <div className="absolute inset-0 bg-[#D34E4E] opacity-20 blur-[60px] rounded-full transform scale-150" />
        
        <div className="relative z-10 drop-shadow-[0_0_15px_rgba(255,255,255,0.15)]">
          <Image
      ...
      {/* FADING MESSAGE */}
      <div className="h-12 flex items-center justify-center px-4 max-w-2xl">
        <p 
          className={`text-xl font-light text-white text-center transition-opacity duration-500 ease-in-out ${visible ? 'opacity-100' : 'opacity-0'}`}
        >
      ...
      <div className="mt-8 flex gap-2">
        <div className="w-2 h-2 bg-white/30 rounded-full animate-[pulse_1.5s_infinite_0s]" />
        <div className="w-2 h-2 bg-white/30 rounded-full animate-[pulse_1.5s_infinite_0.3s]" />
        <div className="w-2 h-2 bg-white/30 rounded-full animate-[pulse_1.5s_infinite_0.6s]" />
      </div>
